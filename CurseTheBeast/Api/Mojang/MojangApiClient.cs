...
public class MojangApiClient : BaseApiClient
{
    ...
    { 
        return await GetAsync<GameVersions>(new Uri($"https://piston-meta.mojang.com/mc/game/version_manifest.json"), null, ct);
    }

    public async Task<GameManifest> GetGameManifestAsync(string manifestUrl, CancellationToken ct = default)
    {
        return await GetAsync<GameManifest>(new Uri(manifestUrl), null, ct);
    }
}
